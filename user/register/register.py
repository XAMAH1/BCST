...
def user_register(connect):
   try:
       ...
       command = insert(autme_user).values(login=data["login"], password=password, date_create=str(date_today), IMEI=data["IMEI"])
       connect.execute(command)
       connect.commit()
       command = select(autme_user).where(autme_user.c.login == data["login"])
       result = connect.execute(command)
       for new_user in result:
           command = insert(user).values(autme_id=int(new_user[0]), fio=data["fio"], mail=data["mail"], phone=data["phone"])
           connect.execute(command)
           connect.commit()
